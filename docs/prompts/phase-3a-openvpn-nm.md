...
2. Does `nmcli device show tun0` expose DNS in the same `IP4.DNS[n]` fields? Capture sample output and add it as a parser test fixture
...
- Add OpenVPN `nmcli device show` output as a new fixture in the detector parser tests (proves DNS parsing covers the OpenVPN field layout)
...
PR into `dev` titled `Phase 3a: OpenVPN via NetworkManager`. Description: investigation findings (which signal(s) are authoritative, with `nmcli`/`busctl` evidence), what changed, manual verification log, done-criteria checklist.
